...
class PMInfo:
    ...
    """
    def __init__(self):
        self.smallest_pivot = np.inf
        self.number_of_times_ppivoted = 0
        self.warn_not_odf = False
        self.warn_under_determined = False
        self.warn_inconsistent = False
...
def mat2pm(a: np.array, thresh=1e-5):
    ...
    :return: Tuple of (array of principal minors: np.array, computation info: PMInfo).
    ...
    info.number_of_times_ppivoted = len(zeropivs)
    return pm, info


def pm_info_to_string(info: PMInfo):
    """
    Returns a human readable rendering (String) of the PMInfo class.
    :param info: Input PMInfo class
    :return: Human readable string
    """
    s = f'Pseudo-pivoted {info.number_of_times_ppivoted} times, smallest pivot used: {info.smallest_pivot}'
...
def pm2mat(pm: np.array):
    ...
    :param pm: The structure of pm, where |a[v]| is the principal minor of "a" indexed by the vector v is:
        pm: |a[1]|, |a[2]|, |a[1 2]|, |a[3]|, |a[1 3]|, |a[2 3]|, |a[1 2 3]|,...
    :return: Tuple of (a: np.array, computation info: PMInfo)
    """

    def _invschurc(_info: PMInfo, _pivot, _l, _r):
        """
        ...
        if m == 2:
            [t1, t2] = _solveright(_info, _l[0, 0], _l[0, 1], _l[1, 0], _l[1, 1], _r[0, 0], _r[0, 1], _r[1, 0],
                                   _r[1, 1])
            ...
            i2 = m - 2
            i3 = m - 1
            [r1, r2] = _solveright(_info, _l[i2, i2], _l[i2, i3], _l[i3, i2], _l[i3, i3], _r[i2, i2], _r[i2, i3],
                                   _r[i3, i2],
                                   _r[i3, i3])
            [s1, s2] = _solveright(_info, _l[i1, i1], _l[i1, i2], _l[i2, i1], _l[i2, i2], _r[i1, i1], _r[i1, i2],
                                   _r[i2, i1],
                                   _r[i2, i2])
            [t1, t2] = _solveright(_info, _l[i1, i1], _l[i1, i3], _l[i3, i1], _l[i3, i3], _r[i1, i1], _r[i1, i3],
                                   _r[i3, i1],
                                   _r[i3, i3])
            ...
            for i1 in range(m - 4, -1, -1):
                ...
                for _j in range(i1 + 2, m):
                    ...
                    j1 = i1 + 1
                    [t1, t2] = _solveright(_info, _l[i1, i1], _l[i1, _j], _l[_j, i1], _l[_j, _j], _r[i1, i1],
                                           _r[i1, _j],
                                           _r[_j, i1], _r[_j, _j])
    ...
    def _solveright(_info: PMInfo, _l11, _l12, _l21, _l22, _r11, _r12, _r21, _r22):
        """
        ...
        _x2 = _l22 - _r22
        _d = np.sqrt(
            _x1 * _x1 * _x2 * _x2 + _l12 * _l12 * _l21 * _l21 + _r12 * _r12 * _r21 * _r21 - 2 * _x1 * _x2 * _l12 * _l21
    ...
    for level in range(n - 2, -1, -1):  # just counts? = n-2:-1:0        # for consistency with mat2pm levels
        ...
        for i in range(nq - 1, -1, -1):  # = nq:-1:1     # process matrices in reverse order for zeropivs
            if i == 0:
                ...
                pivot = _pm[ipm1] / _pm[ipm2]
                ipm2 -= 1
            info.smallest_pivot = min(info.smallest_pivot, np.abs(pivot))
            qq[i] = _invschurc(info, pivot, q[i], q[i + nq])
            if zeropivsmax == ipm1:
                qq[i][0][0] -= ppivot
                zeropivsmax = zeropivs[zeropivsidx]
                zeropivsidx -= 1
